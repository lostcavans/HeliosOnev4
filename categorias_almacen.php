    ...
    switch ($action) {
        case 'create':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $stmt = $pdo->prepare("INSERT INTO categorias_almacen (nombre, descripcion, estado) VALUES (?, ?, ?)");
                $stmt->execute([
                    $_POST['nombre'],
                    $_POST['descripcion'],
                    $_POST['estado']
        ...
        case 'edit':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $stmt = $pdo->prepare("UPDATE categorias_almacen SET nombre=?, descripcion=?, estado=? WHERE id_categoria=?");
                $stmt->execute([
                    $_POST['nombre'],
                    $_POST['descripcion'],
                    $_POST['estado'],
        ...
        case 'delete':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM almacen WHERE id_categoria = ?");
                $stmt->execute([$id]);
                $result = $stmt->fetch();
                ...
                if ($result['total'] > 0) {
                    ...
                    echo json_encode([
                        'success' => false, 
                        'message' => 'No se puede eliminar: Hay items asociados a esta categoría'
                    ]);
                    exit;
                }
                
                $stmt = $pdo->prepare("DELETE FROM categorias_almacen WHERE id_categoria = ?");
                $stmt->execute([$id]);
...
if (in_array($action, ['edit', 'view'])) {
    $stmt = $pdo->prepare("SELECT * FROM categorias_almacen WHERE id_categoria = ?");
    $stmt->execute([$id]);
    $categoria = $stmt->fetch();
    
    if (!$categoria) {
        header('Location: categorias_almacen.php');
        exit;
    }
}
...
<head>
    ...
    <title>Categorías de Almacén - Sistema Bomberos</title>
...
<body>  
        ...
        <div class="container-fluid">
            <div class="page-header">
                <h1 class="text-titles">
                    <i class="fas fa-tags"></i> <?= ucfirst($action) ?> Categoría de Almacén
                </h1>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="text-end">
                            <a href="categorias_almacen.php" class="btn btn-secondary">Cancelar</a>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="text-end">
                        <a href="categorias_almacen.php" class="btn btn-secondary">Volver</a>
                        <a href="categorias_almacen.php?action=edit&id=<?= $categoria['id_categoria'] ?>" class="btn btn-warning">Editar</a>
                    </div>
                </div>
            </div>
            ...
            <?php else: ?>
            <div class="mb-4">
                <a href="categorias_almacen.php?action=create" class="btn btn-primary">
                    ...
                    <table id="categorias-table" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="categorias_almacen.php?action=view&id=<?= $cat['id_categoria'] ?>" class="btn btn-info btn-sm">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="categorias_almacen.php?action=edit&id=<?= $cat['id_categoria'] ?>" class="btn btn-warning btn-sm">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-eliminar', function() {
            ...
            $.ajax({
                url: 'categorias_almacen.php?action=delete&id=' + id,
                type: 'POST',
                dataType: 'json'
            })
